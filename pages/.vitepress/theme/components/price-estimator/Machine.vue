...
export default {
  ...
  computed: {
    ...
    getComputePriceYear() {
      ...
      const price = this.flavors.find(
        (item) =>
          ...
          item["service.level"] === this.formData.subscription
      );
    ...
    getGpuPrice() {
      ...
      const price = this.gpus.find(
        (item) =>
          item["service.unit"] === this.formData.gpu &&
          item["service.level"] === this.getGpuSubscription
      );
    ...
    getGpuSubscription() {
      ...
      return this.formData.subscription
        ? this.formData.subscription
          .replace("SPOT", "ONDEMAND")
          .replace("COMMITMENT", "ONDEMAND")
        : this.formData.subscription;
    },
    getGpuMonthPrice() {
  ...
  methods: {
    ...
    save() {
      ...
      let monthlyPrice = this.getSummedPrice(
        this.getComputePriceMonth,
        this.getGpuMonthPrice
      );
      ...
      const flavorWithGpu = this.formData.gpu
        ? this.formData.flavor + " + " + this.formData.gpu
    ...
    <v-card-text>
      <v-container>
        <v-row dense>
          ...
          <v-col cols="12">
            <v-autocomplete v-model="formData.subscription" :items="subscriptions" item-title="text" item-value="value"
              label="Subscription type" required variant="outlined">
            </v-autocomplete>
          </v-col>
          <v-col cols="12">
            <v-autocomplete v-model="formData.flavor" :items="getFlavors" label="Machine type" variant="outlined"
              required :disabled="!formData.subscription">
          ...
          <v-col v-show="formData.flavor" cols="12" sm="6">
            <v-text-field v-model="getComputePriceMonth" label="Compute Price / Month" suffix="NOK ex. VAT" readonly
              variant="outlined"></v-text-field>
          </v-col>
          <v-col v-show="formData.flavor" cols="12" sm="6">
            <v-text-field v-model="getComputePriceYear" label="Compute Price / Year" suffix="NOK ex. VAT / Year"
              readonly variant="outlined"></v-text-field>
          </v-col>
          <v-col cols="12">
            <v-select v-model="formData.gpu" :items="getGpus" label="GPU type (optional)" variant="outlined" clearable
              :disabled="!formData.subscription"></v-select>
          </v-col>
          <v-col v-show="formData.gpu" cols="12" sm="6">
            <v-text-field v-model="getGpuMonthPrice" label="GPU Price / Month" suffix="NOK ex. VAT" readonly
              variant="outlined"></v-text-field>
          </v-col>
          <v-col v-show="formData.gpu" cols="12" sm="6">
            <v-text-field v-model="getGpuPrice" label="GPU Price / Year" suffix="NOK ex. VAT / Year" readonly
              variant="outlined"></v-text-field>
          </v-col>
        </v-row>
      </v-container>
